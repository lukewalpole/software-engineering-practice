...
public class AssignedTechFilter extends JFrame {
	...
	public AssignedTechFilter(TaskAllocationApp newApp) {
		app=newApp;
		...
		btnFindButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(selectedRow>=0) {
			      ...
			      // set the main window to default
			      DefaultTableModel tableModel = (DefaultTableModel)app.theMainWindow.table.getModel();
				  tableModel.setRowCount(0);
				  for(Task t:app.data.getAllTasks()) //iterate through all tasks
		...
		text_searchField.addKeyListener(new KeyAdapter() {
			...
			public void keyReleased(KeyEvent e) {
				...
				for(Task tsk:app.data.getAllTasks()) // iterate through all tasks
				  for(Technician tech:app.data.getAllTech())//iterate through all technicians
	...
	void displayTechs() {
		  DefaultTableModel tableModel = (DefaultTableModel) techTable.getModel();
		  tableModel.setRowCount(0);
		for(Task s:app.data.getAllTasks()) {
		  for(Technician t:app.data.getAllTech()) {
