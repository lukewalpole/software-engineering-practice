						...
						// allow access based on job title
							if(user.getUserJob().equals("HR")) {
							 ...
							 if(user.getUserJob().equals("Tech")) {
								 TaskAllocationApp app=  new TaskAllocationApp(launcher.database, launcher);
								  app.setUserIDLog(user.getUserID());
								  
								  //pass the username to Task7.TaskAllocationComponent
								  app.setUserName(username);
								 dispose();
							 }
							if(user.getUserJob().equals("Cs")) {
								VSSApp motorApp= new VSSApp();
								 dispose();
							 }
