...
public class HRAccount extends JFrame {
	...
	public HRAccount(LogInLauncher newLogIn) {
		...
		btnAddButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				//Call AddUser to add a new User
				AddUser newUser= new AddUser(launcher);
		...
		btnEditButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				 ...
				 if(selectedRow>=0) {
					...
					//Call EditUser to add a new User
					EditUser userEdit= new EditUser(launcher);
					...
					for(UserAccount user:launcher.dbQuery.getUserAccountInfo()) {
						...
						userEdit.userID=user.getUserID();
						userEdit.textFieldUsername.setText(user.getUsername());
						userEdit.textFieldPassword.setText(user.getPassword());
						userEdit.textFieldFirstName.setText(user.getUserFirstName());
						userEdit.textFieldLastName.setText(user.getUserLastName());
						userEdit.comboBoxUserJob.setSelectedItem(user.getUserJob());
						}
					}
				 }
			}
						...
						if(user.getUserID()==selectedUserID) {
							...
							int result = JOptionPane.showConfirmDialog(null,"Do you want to Delete '"+user.getUsername()+"'?", "CONFIRMATION", JOptionPane.YES_NO_OPTION,JOptionPane.QUESTION_MESSAGE);
					            if(result == JOptionPane.YES_OPTION){
					            	launcher.removeUser(selectedUserID);// pass the id to LogInLauncher
					            }
						}
					}
				}
